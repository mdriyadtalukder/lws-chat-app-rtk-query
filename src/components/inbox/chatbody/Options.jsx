...
import { useEditCoversationMutation } from "../../../features/conversation/conversationApi";
import { useSelector } from "react-redux";

export default function Options({ info }) {
    const [message, setMessage] = useState('');
    const [editCoversation, { data, isSuccess }] = useEditCoversationMutation();
    const { user } = useSelector((state) => state?.auth);
    const participant = info?.receiver?.email !== user?.email ? info?.receiver : info?.sender;
    //submit
